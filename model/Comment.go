...
func GetComment(id int) (Comment, int) {
	var comment Comment
	dberr = db.Where("id = ?", id).First(&comment).Error
	if dberr != nil {
...
func GetCommentList(pageSize int, pageNum int) ([]Comment, int64, int) {
	...
	dberr = db.Model(&commentList).Limit(pageSize).Offset((pageNum - 1) * pageSize).Order("Created_At DESC").Select("comment.id, article.title,user_id,article_id, user.username, comment.content, comment.status,comment.created_at,comment.deleted_at").Joins("LEFT JOIN article ON comment.article_id = article.id").Joins("LEFT JOIN user ON comment.user_id = user.id").Scan(&commentList).Error
	if dberr != nil {
...
func GetCommentListFront(id int, pageSize int, pageNum int) ([]Comment, int64, int) {
	...
	var total int64
	db.Find(&Comment{}).Where("article_id = ?", id).Where("status = ?", 1).Count(&total)
	dberr = db.Model(&Comment{}).Limit(pageSize).Offset((pageNum-1)*pageSize).Order("Created_At DESC").Select("comment.id, article.title, user_id, article_id, user.username, comment.content, comment.status,comment.created_at,comment.deleted_at").Joins("LEFT JOIN article ON comment.article_id = article.id").Joins("LEFT JOIN user ON comment.user_id = user.id").Where("article_id = ?",
...
func DeleteComment(id uint) int {
	var comment Comment
	dberr = db.Where("id = ?", id).Delete(&comment).Error
	if dberr != nil {
		return errmsg.ERROR
	}
	return errmsg.SUCCSE
...
func CheckComment(id int, data *Comment) int {
	...
	dberr = db.Model(&comment).Where("id = ?", id).Updates(maps).First(&res).Error
