<template>
  <a-layout-sider breakpoint="lg" v-model="collapsed">
    <div class="logo">
      <!-- <span>{{collapsed ? "温迪" : "※温迪※ 的个人博客"}}</span> -->
      <div v-if="collapsed">
        <img src="http://test.scjglaw.cn/Fm63wdxiVmW7xN3z2R-czYUdMmLY" alt="" style="width: 48px;height: 48px;border-radius: 50%;">
      </div>
      <div v-else>
        <span>{{collapsed ? "温迪" : "※温迪※ 的个人博客"}}</span>
      </div>
    ...
    <a-menu theme="dark" mode="inline" @click="goToPage">
      ...
      <a-menu-item key="commentlist">
        <a-icon type="message" /><span>评论管理</span>
      </a-menu-item>
      <!-- 用户列表 -->
